...
import { 
  ...
  Smartphone, CheckCircle2, Search, PlusCircle, X, Link as LinkIcon 
} from 'lucide-react'
...
export default function AdminPortal() {
  ...
  // FIXED: Variable naming for filtering to avoid ReferenceError
  ...
  if (!isLoggedIn) return (
    <div className="min-h-screen bg-black flex items-center justify-center p-6">
      <div className="bg-[#111] p-10 rounded-[2.5rem] border border-white/5 w-full max-w-sm text-center shadow-2xl transition-all">
        ...
        <button onClick={() => passwordInput === dbPassword ? setIsLoggedIn(true) : alert("Invalid Code!")} className="w-full bg-[#2ea64d] text-white font-bold py-4 rounded-xl uppercase text-[10px]">Access Dashboard</button>
      </div>
    </div>
  )
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="flex justify-between items-center mb-10 pb-6 border-b dark:border-white/5">
          ...
          <div className="flex gap-4">
            ...
            <button onClick={() => setIsLoggedIn(false)} className="p-2 bg-gray-100 dark:bg-white/5 rounded-xl border dark:border-white/5 hover:text-red-500 transition-all"><LogOut size={20}/></button>
          </div>
        </header>
        ...
        <div className="flex gap-4 mb-10 overflow-x-auto pb-4 no-scrollbar border-b dark:border-white/5">
          {[ 
            ...
            {id:'reports', l:'Reports', i:AlertTriangle, c:'bg-red-600'} 
          ].map(t => (
        ...
        {view === 'dashboard' && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 animate-in fade-in">
             {[ {l:'Orbit Apps', v:stats.apps, i:Package, c:'text-[#2ea64d]'}, {l:'User Requests', v:stats.requests, i:Activity, c:'text-blue-500'}, {l:'Orbit Blogs', v:articles.length, i:PenTool, c:'text-orange-500'}, {l:'Bug Reports', v:stats.reports, i:AlertTriangle, c:'text-red-500'} ].map(s => (
