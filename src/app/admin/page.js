...
import { 
  ...
  Smartphone, CheckCircle2, AlertCircle, LogOut, Star // FIXED: Added Star here
} from 'lucide-react'
...
export default function AdminPortal() {
  ...
  const saveApp = async (e) => {
    ...
    if (formData.id) {
      ...
      const insertPayload = { ...finalData };
      delete insertPayload.id;
      res = await supabase.from('apps').insert([insertPayload])
    }
  ...
  if (!isLoggedIn) return (
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6">
      <div className="bg-[#111] p-10 rounded-[2.5rem] border border-white/5 w-full max-w-sm text-center shadow-2xl">
        ...
        <h2 className="text-white font-black text-xl mb-8 uppercase italic tracking-tighter">Authorize Orbit</h2>
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] dark:bg-[#0a0a0a] text-gray-900 dark:text-white p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-4 mb-10 overflow-x-auto pb-2 no-scrollbar">
          <button onClick={() => setView('dashboard')} className={`px-6 py-2 rounded-full font-bold text-[10px] uppercase tracking-widest whitespace-nowrap transition-all ${view === 'dashboard' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-white/5'}`}>Analytics</button>
          <button onClick={() => setView('apps')} className={`px-6 py-2 rounded-full font-bold text-[10px] uppercase tracking-widest whitespace-nowrap transition-all ${view === 'apps' ? 'bg-[#2ea64d] text-white' : 'bg-gray-100 dark:bg-white/5'}`}>App Manager</button>
          <Link href="/admin/requests" className="px-6 py-2 rounded-full font-bold text-[10px] uppercase tracking-widest whitespace-nowrap bg-gray-100 dark:bg-white/5 hover:text-blue-500 transition-all">Requests</Link>
        </div>
          ...
          <div className="animate-in fade-in duration-500">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
               ...
               <div className="bg-white dark:bg-[#111] p-8 rounded-[2.5rem] border dark:border-white/5 text-center shadow-sm">
                  ...
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Feedbacks</p>
            ...
            <div className="bg-white dark:bg-[#111] p-8 rounded-[3rem] border dark:border-white/5 shadow-sm">
               <h3 className="text-sm font-black uppercase mb-8 flex items-center gap-2 italic tracking-widest text-gray-400"><BarChart3 size={18}/> Status Snapshot</h3>
               <div className="grid grid-cols-1 gap-4">
                  {apps.slice(0, 5).map(app => (
                    <div key={app.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-black/20 rounded-2xl">
                       ...
                       <span className="text-[8px] font-black uppercase px-3 py-1 bg-[#2ea64d]/10 text-[#2ea64d] rounded-full">Online</span>
                    </div>
                  ))}
               </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-1">
               <form onSubmit={saveApp} className="bg-white dark:bg-[#111] p-8 rounded-[2.5rem] border dark:border-white/5 shadow-xl sticky top-4 space-y-4">
                  <h3 className="text-[10px] font-black uppercase text-[#2ea64d] mb-4 border-b dark:border-white/5 pb-2">Launch Protocol</h3>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                     ...
                     <input className="w-full bg-gray-50 dark:bg-black border dark:border-white/10 p-3 rounded-xl text-xs outline-none" placeholder="Version" value={formData.version} onChange={e => setFormData({...formData, version: e.target.value})} />
                  </div>
                  <div className="flex items-center justify-between p-3 border dark:border-white/10 rounded-xl bg-gray-50 dark:bg-black text-[9px] font-bold text-gray-500 uppercase italic">
                    <span>Free Access?</span>
                    <input type="checkbox" checked={isFree} onChange={() => setIsFree(!isFree)} className="w-4 h-4 accent-[#2ea64d]" />
                  </div>
                  {!isFree && <input required className="w-full bg-gray-50 dark:bg-black border dark:border-white/10 p-3 rounded-xl text-xs outline-none font-black text-[#2ea64d]" placeholder="Price" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} />}
                  <textarea required className="w-full bg-gray-50 dark:bg-black border dark:border-white/10 p-3 rounded-xl text-xs outline-none" placeholder="Description" rows="2" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
                  <div className="space-y-2">
                     <div className="flex items-center justify-center p-3 border border-dashed dark:border-white/10 rounded-xl relative hover:border-[#2ea64d] transition-all cursor-pointer">
                        <Upload size={14} className="mr-2 text-[#2ea64d]"/><span className="text-[9px] font-black uppercase text-gray-500">{formData.download_url.includes('supabase') ? 'Ready ✅' : 'Drop APK'}</span>
                        <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={e => handleFileUpload(e, 'apks', 'apk')} />
                     </div>
                     <input className="w-full bg-gray-50 dark:bg-black border dark:border-white/10 p-3 rounded-xl text-[10px] outline-none" placeholder="Manual URL..." value={formData.download_url} onChange={e => setFormData({...formData, download_url: e.target.value})} />
                  </div>
                  <button disabled={loading} className="w-full bg-[#2ea64d] text-white font-black py-4 rounded-xl uppercase text-[10px] shadow-lg active:scale-95 transition-all">
                    {loading ? 'Working...' : 'Synchronize App'}
                  </button>
              ...
              {apps.map(app => (
                <div key={app.id} className="bg-white dark:bg-[#111] p-4 rounded-2xl border dark:border-white/5 flex items-center justify-between group hover:border-[#2ea64d] transition-all shadow-sm">
                  ...
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    ...
                    <button onClick={async () => { if(confirm("Delete App?")){ await supabase.from('apps').delete().eq('id', app.id); fetchData(); } }} className="p-2 bg-red-500/10 text-red-500 rounded-lg"><Trash2 size={16}/></button>
                  </div>
                </div>
              ))}
            </div>
