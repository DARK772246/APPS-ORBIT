...
import { 
  ...
  Star, PenTool, Image as ImageIcon, LogOut, Layout, AlertTriangle, Smartphone, CheckCircle2 
} from 'lucide-react'
...
export default function AdminPortal() {
  ...
  const saveApp = async (e) => {
    ...
    if (!res.error) { setMessage({text:'Orbit Updated!', type:'success'}); resetAppForm(); fetchData(); }
    setLoading(false)
  }
  ...
  const saveArticle = async (e) => {
    ...
    if (!res.error) { setMessage({text:'Article Published!', type:'success'}); setArtData({id:null, title:'', content:'', author:'Salman Khan', writer:'Salman AppOrbit', image_url:''}); fetchData(); }
    setLoading(false)
  }
  ...
  if (!isLoggedIn) return (
    <div className="min-h-screen bg-black flex items-center justify-center p-6">
      <div className="bg-[#111] p-10 rounded-[2.5rem] border border-white/5 w-full max-w-sm text-center shadow-2xl">
        <h2 className="text-white font-black text-xl mb-8 uppercase italic tracking-widest text-[#2ea64d]">Admin Orbit</h2>
        ...
        <button onClick={() => passwordInput === dbPassword ? setIsLoggedIn(true) : alert("Invalid!")} className="w-full bg-[#2ea64d] text-white font-bold py-4 rounded-xl uppercase text-[10px]">Access Dashboard</button>
      </div>
    </div>
  )

  return (
    <div className="min-h-screen bg-white dark:bg-[#0a0a0a] text-gray-900 dark:text-white p-6 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        {view === 'apps' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 animate-in fade-in">
             <form onSubmit={saveApp} className="bg-gray-50 dark:bg-[#111] p-8 rounded-[2rem] border dark:border-white/5 space-y-4 shadow-sm h-fit">
                ...
                <button disabled={loading} className="w-full bg-[#2ea64d] text-white font-black py-4 rounded-xl uppercase text-[10px]">Publish</button>
             </form>
             <div className="lg:col-span-2 space-y-4">
                {apps.map(app => (
                  <div key={app.id} className="bg-gray-50 dark:bg-[#111] p-4 rounded-2xl flex justify-between items-center border dark:border-white/5 group hover:border-[#2ea64d] transition-all">
        ...
        {view === 'slider' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 animate-in fade-in">
             <form onSubmit={saveSlide} className="bg-gray-50 dark:bg-[#111] p-8 rounded-[2rem] border dark:border-white/5 space-y-4 h-fit">
                ...
                <button className="w-full bg-purple-600 text-white font-black py-4 rounded-xl uppercase text-[10px]">Publish Slide</button>
             </form>
             <div className="lg:col-span-2 space-y-4">
                {slides.map(s => (<div key={s.id} className="p-4 bg-gray-50 dark:bg-[#111] rounded-xl flex justify-between items-center border dark:border-white/5 group shadow-sm">
                   <h4 className="font-bold text-xs uppercase italic">{s.title}</h4>
                   <button onClick={async () => { await supabase.from('featured_slides').delete().eq('id', s.id); fetchData(); }} className="text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={16}/></button>
                </div>))}
             </div>
          </div>
        )}
        ...
        {view === 'blog' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 animate-in fade-in">
             <form onSubmit={saveArticle} className="bg-gray-50 dark:bg-[#111] p-8 rounded-[2rem] border dark:border-white/5 space-y-4 h-fit shadow-sm">
                ...
                <button className="w-full bg-orange-500 text-white font-black py-4 rounded-xl uppercase text-[10px]">Publish</button>
             </form>
             <div className="lg:col-span-2 space-y-4">
        ...
        {view === 'reports' && (
          <div className="space-y-4 animate-in fade-in"><h3 className="text-xl font-black uppercase text-red-500 italic mb-6">Orbit Reports</h3>{reports.map(r => (<div key={r.id} className="bg-white dark:bg-[#111] p-6 rounded-[2.5rem] border border-red-500/10 flex justify-between items-center group shadow-sm"><div><h4 className="font-black text-[#2ea64d] uppercase text-sm">{r.app_name}</h4><p className="text-xs text-red-500 font-bold uppercase mt-1 italic">"{r.issue_type}"</p></div><button onClick={async () => { await supabase.from('reports').delete().eq('id', r.id); fetchData(); }} className="text-gray-500 hover:text-red-500 transition-colors"><Trash2 size={20}/></button></div>))}{reports.length === 0 && <p className="text-gray-500 text-center italic py-20 font-bold uppercase text-[10px] tracking-widest opacity-20">Clear Orbit.</p>}</div>
        )}
