...
import { motion } from 'framer-motion' // Fixed: Missing import
import { 
...
export default function AppDetail({ params: paramsPromise }) {
  ...
  async function fetchData() {
    const { data: appData, error } = await supabase.from('apps').select('*').eq('id', id).single()
    if (appData) {
      setApp(appData)
      // Dynamic Title
  ...
  if (!app) return <div className="min-h-screen bg-white dark:bg-[#0a0a0a] flex items-center justify-center text-[#24cd77] font-black animate-pulse">Establishing Orbit...</div>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0a0a0a] text-gray-900 dark:text-white transition-colors duration-300 font-sans">
      <nav className="p-4 border-b border-gray-100 dark:border-white/5 flex justify-between items-center sticky top-0 bg-white/90 dark:bg-[#0a0a0a]/90 backdrop-blur-md z-50 px-6">
        <Link href="/" className="text-[10px] font-black uppercase text-gray-500 hover:text-[#24cd77] flex items-center gap-2">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
        ...
        {isRecentlyUpdated && (
          <motion.div initial={{y:-20, opacity:0}} animate={{y:0, opacity:1}} className="bg-blue-600 text-white p-4 rounded-2xl mb-8 flex items-center gap-3 shadow-lg">
            <Bell className="animate-bounce" size={20}/>
            <p className="text-[10px] font-black uppercase tracking-widest">New Update: v{app.version} is now live!</p>
          </motion.div>
        )}
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            {scanning ? (
              <div className="inline-flex items-center gap-3 bg-blue-500/5 px-6 py-3 rounded-2xl mb-8 animate-pulse">
                ...
                <p className="text-[10px] font-black uppercase text-blue-500 tracking-widest italic">Orbit Scanning...</p>
              </div>
            ) : (
        ...
        <div className="bg-white dark:bg-[#111] border border-gray-100 dark:border-white/5 rounded-[2rem] p-6 mb-12">
        ...
        {showPayment && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/90 backdrop-blur-md">
            <div className="bg-white dark:bg-[#111] p-10 rounded-[3rem] max-w-sm w-full relative border border-[#24cd77]/30 text-center animate-in zoom-in-95">
              ...
              <h3 className="text-xl font-black uppercase mb-8 italic tracking-tighter italic">Orbit <span className="text-[#24cd77]">Payment</span></h3>
              <div className="space-y-4 mb-8">
                ...
                <div className="p-5 bg-gray-50 dark:bg-black/40 rounded-3xl border border-gray-100 dark:border-white/5 text-left italic">
                  <p className="text-[9px] font-bold text-[#24cd77] uppercase mb-1">NayaPay IBAN</p>
                  <p className="text-[10px] font-bold break-all font-mono italic">PK16 NAYA 1234 5032 7517 6283</p>
                </div>
              </div>
