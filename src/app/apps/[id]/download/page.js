...
export default function DownloadPage({ params: paramsPromise }) {
  ...
  if (!app) return <div className="min-h-screen bg-white dark:bg-[#0a0a0a] flex items-center justify-center text-[#24cd77] font-bold">Orbiting Link...</div>
    ...
    <div className="min-h-screen bg-[#f8f9fa] dark:bg-[#0a0a0a] text-gray-900 dark:text-white transition-colors duration-300 font-sans">
      ...
      <main className="max-w-2xl mx-auto px-6 py-20 text-center">
        <div className="w-24 h-24 bg-gray-100 dark:bg-[#121212] rounded-3xl mx-auto mb-6 border border-gray-200 dark:border-white/10 overflow-hidden shadow-xl flex items-center justify-center">
          {app.icon_url ? <img src={app.icon_url} className="w-full h-full object-cover" /> : <span className="text-4xl">📱</span>}
        </div>
        <h1 className="text-2xl font-black uppercase tracking-tight mb-2">Preparing Link</h1>
        
        <div className="bg-white dark:bg-[#111] p-12 rounded-[3.5rem] border border-gray-100 dark:border-white/5 mb-8 shadow-sm">
          {!ready ? (
            <div className="flex flex-col items-center">
               ...
               <p className="text-[10px] font-black uppercase text-gray-400 mt-4 tracking-widest italic flex items-center gap-2"><ShieldCheck size={14}/> Scanning for Viruses</p>
            </div>
          ) : (
            <div className="animate-in zoom-in-95 duration-500">
               <p className="text-[#24cd77] font-black mb-6 uppercase tracking-widest text-sm">Secure Link Ready!</p>
               <a href={app.download_url} target="_blank" rel="noopener noreferrer" className="block bg-[#24cd77] hover:bg-[#268a40] text-white font-black py-5 rounded-2xl uppercase text-xs shadow-xl active:scale-95 transition-all">Download APK Now</a>
            </div>
          )}
        </div>
      </main>
