...
function CommentsSection({ appId }) {
  ...
  return (
    <div className="mt-20">
      ...
      <form onSubmit={handleSubmit} className="glass-panel p-8 rounded-[3rem] border border-white/5 mb-10">
        ...
        <textarea required rows="3" placeholder="Data log..." className="w-full bg-black/40 p-4 rounded-2xl text-xs mb-4 outline-none border border-white/5 text-white italic" value={commentText} onChange={(e) => setCommentText(e.target.value)} />
...
export default function AppDetailPage({ params: paramsPromise }) {
  ...
  if (!app) return <div className="min-h-screen bg-[#020617] flex items-center justify-center text-white font-black italic uppercase">LOG: NOT FOUND</div>;
    ...
    <div className="min-h-screen bg-[#020617] text-white font-sans pb-20">
      ...
      <main className="max-w-6xl mx-auto px-6 pt-32 lg:pt-40">
        ...
        <div className="max-w-4xl mx-auto bg-white/5 p-10 rounded-[3rem] border border-white/5 shadow-inner mb-24 relative overflow-hidden">
            ...
            <h3 className="text-xl font-black uppercase italic mb-6 border-l-4 border-blue-500 pl-4 text-blue-500">Mission Overview</h3>
        ...
        {showDownloadModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[600] flex items-center justify-center p-6 bg-black/95 backdrop-blur-3xl">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="glass-panel p-10 md:p-12 rounded-[4rem] max-w-sm w-full text-center relative border-white/10 shadow-2xl">
              ...
              <div className="bg-white/5 p-12 rounded-[3.5rem] mb-8 shadow-inner border border-white/5">
                {!isReady ? (
                  ...
                  <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="animate-in fade-in zoom-in">
                    ...
                    {/* 🚀 THE BULLETPROOF DOWNLOAD LINK FIX 🚀 */}
                    <a 
                      href={app.download_url} 
                      target="_blank" 
                      rel="noopener noreferrer" 
                      download
                      className="block w-full bg-[#2ea64d] hover:bg-[#268a40] text-white font-black py-5 rounded-2xl uppercase text-[12px] shadow-2xl shadow-green-500/40 active:scale-95 transition-all text-center italic tracking-widest"
                    >
                      Download APK Now
                    </a>
                  </motion.div>
              ...
              <p className="text-[9px] text-gray-500 font-black uppercase tracking-widest italic text-center">Orbit Verified: 100% Stable</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
