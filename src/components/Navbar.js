      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
            ...
            className="fixed inset-0 top-16 bg-white dark:bg-[#0a0a0a] z-[150] lg:hidden p-6 flex flex-col gap-6"
          >
            {/* Mobile Search Bar */}
            <div className="relative mb-4">
              <input 
                type="text" 
                placeholder="Search..." 
                className="w-full bg-gray-100 dark:bg-white/5 p-4 rounded-2xl outline-none text-sm dark:text-white" 
                onChange={(e) => {
                   onSearch && onSearch(e.target.value);
                   // Menu band nahi karenge taake results niche nazar aayein
                }} 
              />
            ...
            <div className="flex flex-col gap-4">
              {navLinks.map((link) => (
                <Link 
                  ...
                  className="text-3xl font-black uppercase italic tracking-tighter text-gray-800 dark:text-white hover:text-[#2ea64d] transition-all"
                >
                  {link.name}
                </Link>
              ))}
            ...
            <div className="mt-auto pb-10 border-t dark:border-white/5 pt-6">
